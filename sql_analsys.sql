...
select * from orders;
-- Time between orders (repeat purchase)
...
select pageview_url, count(*) from website_pageviews group by pageview_url;
...
-- 1. finding session trend by date

select 
date(created_at) as by_date,
count(website_session_id) as sessions_count
from website_sessions
group by by_date
order by by_date;

-- 2. finding order trend by date
select
date(created_at) as by_date,
